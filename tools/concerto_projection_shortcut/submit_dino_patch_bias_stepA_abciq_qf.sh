...
LOG_PATH="${LOG_PATH:-${POINTCEPT_DATA_ROOT}/logs/abciq/dino_patch_bias_stepA_${PBS_JOBID:-manual}.log}"
...
echo "=== ABCI-Q DINO patch bias Step A' ==="
echo "date=$(date -Is)"
echo "pbs_jobid=${PBS_JOBID:-}"
echo "workdir=$(pwd -P)"
echo "venv=${VENV_DIR}"
echo "python=$(command -v python)"
echo "out_dir=${OUT_DIR:-${POINTCEPT_DATA_ROOT}/runs/dino_patch_bias_stepA/scannet_pic_wall_smoke}"
nvidia-smi -L || true
nvcc --version || true

bash tools/concerto_projection_shortcut/run_dino_patch_bias_stepA.sh

echo "[done] DINO patch bias Step A'"
echo "[log] ${LOG_PATH}"
