...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/cross_model_residual_fusion_scannet20_reduced_${PBS_JOBID:-manual}.log"
exec > >(tee -a "${LOG_FILE}") 2>&1

echo "=== Cross-model residual fusion ScanNet20 reduced ==="
echo "date=$(date -Is)"
echo "pbs_jobid=${PBS_JOBID:-}"
echo "repo_root=${REPO_ROOT}"
echo "data_root=${SCANNET_DATA_ROOT:-${SCANNET_EXTRACT_DIR}}"
echo "output_dir=data/runs/cross_model_residual_fusion_scannet20/fullft_default_with_ptv3_reduced"
echo "epochs=20"
echo "sample_points_per_scene=2048"
echo "kl_weights=0.0,0.03"
echo "safe_ce_weights=1.0,4.0"
nvidia-smi -L || true
...
"${PYTHON_BIN}" tools/concerto_projection_shortcut/eval_cross_model_residual_fusion_scannet20.py \
  ...
  --output-dir "data/runs/cross_model_residual_fusion_scannet20/fullft_default_with_ptv3_reduced" \
  --summary-prefix "tools/concerto_projection_shortcut/results_cross_model_residual_fusion_scannet20_fullft_default_with_ptv3_reduced" \
  --max-val-batches "-1" \
  --num-worker "${NUM_WORKER:-4}" \
  --full-scene-chunk-size "${FULL_SCENE_CHUNK_SIZE:-2048}" \
  --sample-points-per-scene "2048" \
  --feature-proj-dim "64" \
  --epochs "20" \
...
echo "[done] cross-model residual fusion ScanNet20 reduced"
echo "[log] ${LOG_FILE}"
