...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/cross_model_region_coherence_scannet20_${PBS_JOBID:-manual}.log"
exec > >(tee -a "${LOG_FILE}") 2>&1

echo "=== Cross-model region coherence ScanNet20 ==="
echo "date=$(date -Is)"
echo "pbs_jobid=${PBS_JOBID:-}"
echo "repo_root=${REPO_ROOT}"
echo "data_root=${SCANNET_DATA_ROOT:-${SCANNET_EXTRACT_DIR}}"
echo "output_dir=${OUTPUT_DIR:-data/runs/cross_model_region_coherence_scannet20/fullft_ptv3}"
echo "max_val_batches=${MAX_VAL_BATCHES:--1}"
echo "region_sizes=${REGION_SIZES:-4,8,16}"
nvidia-smi -L || true

"${PYTHON_BIN}" tools/concerto_projection_shortcut/eval_cross_model_region_coherence_scannet20.py \
  --repo-root "${REPO_ROOT}" \
  --data-root "${SCANNET_DATA_ROOT:-${SCANNET_EXTRACT_DIR}}" \
  --output-dir "${OUTPUT_DIR:-data/runs/cross_model_region_coherence_scannet20/fullft_ptv3}" \
  --summary-prefix "${SUMMARY_PREFIX:-tools/concerto_projection_shortcut/results_cross_model_region_coherence_scannet20_fullft_ptv3}" \
  --max-val-batches "${MAX_VAL_BATCHES:--1}" \
  --num-worker "${NUM_WORKER:-4}" \
  --full-scene-chunk-size "${FULL_SCENE_CHUNK_SIZE:-2048}" \
  --region-sizes "${REGION_SIZES:-4,8,16}" \
  --include-utonia \
  ...
  --default-expert-name "${DEFAULT_EXPERT_NAME:-Concerto fullFT}" \
  ${EXTRA_ARGS:-}

echo "[done] cross-model region coherence ScanNet20"
echo "[log] ${LOG_FILE}"
