...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/cross_model_feature_deferral_scannet20_${PBS_JOBID:-manual}.log"
exec > >(tee -a "${LOG_FILE}") 2>&1

DEFAULT_CACHED_EXPERT="${CACHED_EXPERT:-PTv3_supervised::data/runs/ptv3_v151_raw_probs_scannet20/full}"

echo "=== Cross-model feature deferral ScanNet20 ==="
echo "date=$(date -Is)"
echo "pbs_jobid=${PBS_JOBID:-}"
echo "repo_root=${REPO_ROOT}"
echo "data_root=${SCANNET_DATA_ROOT:-${SCANNET_EXTRACT_DIR}}"
echo "output_dir=${OUTPUT_DIR:-data/runs/cross_model_feature_deferral_scannet20/fullft_default_with_ptv3}"
echo "max_val_batches=${MAX_VAL_BATCHES:--1}"
echo "sample_points_per_scene=${SAMPLE_POINTS_PER_SCENE:-2048}"
echo "feature_proj_dim=${FEATURE_PROJ_DIM:-64}"
...
"${PYTHON_BIN}" tools/concerto_projection_shortcut/eval_cross_model_feature_deferral_scannet20.py \
  --repo-root "${REPO_ROOT}" \
  --data-root "${SCANNET_DATA_ROOT:-${SCANNET_EXTRACT_DIR}}" \
  --output-dir "${OUTPUT_DIR:-data/runs/cross_model_feature_deferral_scannet20/fullft_default_with_ptv3}" \
  --summary-prefix "${SUMMARY_PREFIX:-tools/concerto_projection_shortcut/results_cross_model_feature_deferral_scannet20_fullft_default_with_ptv3}" \
...
echo "[done] cross-model feature deferral ScanNet20"
echo "[log] ${LOG_FILE}"
