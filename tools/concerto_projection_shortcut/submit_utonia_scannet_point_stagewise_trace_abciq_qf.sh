...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/utonia_stagewise_${PBS_JOBID:-manual}.log"
exec > >(tee -a "${LOG_FILE}") 2>&1

echo "=== ABCI-Q Utonia ScanNet point-stagewise trace ==="
echo "date=$(date -Is)"
echo "pbs_jobid=${PBS_JOBID:-}"
echo "repo_root=${REPO_ROOT}"
echo "output_dir=${OUTPUT_DIR:-${REPO_ROOT}/tools/concerto_projection_shortcut/results_utonia_scannet_point_stagewise_trace}"
...
"${PYTHON_BIN}" tools/concerto_projection_shortcut/eval_utonia_scannet_point_stagewise_trace.py \
  --repo-root "${REPO_ROOT}" \
  ...
  --output-dir "${OUTPUT_DIR:-${REPO_ROOT}/tools/concerto_projection_shortcut/results_utonia_scannet_point_stagewise_trace}" \
...
echo "[done] Utonia ScanNet point-stagewise trace"
echo "[log] ${LOG_FILE}"
