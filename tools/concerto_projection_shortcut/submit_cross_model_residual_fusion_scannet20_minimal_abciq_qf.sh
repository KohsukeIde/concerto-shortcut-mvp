...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/cross_model_residual_fusion_scannet20_minimal_${PBS_JOBID:-manual}.log"
exec > >(tee -a "${LOG_FILE}") 2>&1

echo "=== Cross-model residual fusion ScanNet20 minimal ==="
echo "date=$(date -Is)"
echo "pbs_jobid=${PBS_JOBID:-}"
echo "repo_root=${REPO_ROOT}"
echo "data_root=${SCANNET_DATA_ROOT:-${SCANNET_EXTRACT_DIR}}"
echo "output_dir=data/runs/cross_model_residual_fusion_scannet20/fullft_default_with_ptv3_minimal"
echo "epochs=10"
echo "sample_points_per_scene=1024"
echo "kl_weights=0.0"
...
"${PYTHON_BIN}" tools/concerto_projection_shortcut/eval_cross_model_residual_fusion_scannet20.py \
  ...
  --output-dir "data/runs/cross_model_residual_fusion_scannet20/fullft_default_with_ptv3_minimal" \
  --summary-prefix "tools/concerto_projection_shortcut/results_cross_model_residual_fusion_scannet20_fullft_default_with_ptv3_minimal" \
  --max-val-batches "-1" \
...
echo "[done] cross-model residual fusion ScanNet20 minimal"
echo "[log] ${LOG_FILE}"
