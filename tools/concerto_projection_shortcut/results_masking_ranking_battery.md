...
- Missing comparator: fully supervised PTv3 and an external SSL checkpoint are
  not currently in this local ranking table
...
- Clean-to-masked ranking shift is weak with the current comparators. Decoder
  remains above linear for clean, random keep 20%, and structured keep 20%.
  Linear only overtakes decoder under the most extreme random keep 10% row.
- Current claim status: this is a useful evaluation-pilot and an anti
  ...
  meaningful ranking extension is supervised PTv3 and at least one external SSL
  comparator under the same mask variants.
