...
LOG_PATH="${LOG_PATH:-${POINTCEPT_DATA_ROOT}/logs/abciq/scannet_classwise_confusion_${PBS_JOBID:-manual}.log}"
...
echo "=== ABCI-Q ScanNet classwise confusion ==="
echo "date=$(date -Is)"
echo "pbs_jobid=${PBS_JOBID:-}"
echo "workdir=$(pwd -P)"
echo "venv=${VENV_DIR}"
echo "python=$(command -v python)"
echo "out_root=${OUT_ROOT:-${POINTCEPT_DATA_ROOT}/runs/scannet_classwise_diagnosis/large_video_sr_lora}"
nvidia-smi -L || true
nvcc --version || true

bash tools/concerto_projection_shortcut/run_scannet_classwise_confusion.sh

echo "[done] ScanNet classwise confusion"
echo "[log] ${LOG_PATH}"
